...
impl FfmpegApp {
    ...
    fn check_file_picker(&mut self) {
        if let Some(rx) = &self.file_picker_rx {
            // 使用 try_recv 非阻塞地检查是否有结果
            if let Ok(path) = rx.try_recv() {
                // 根据 active_picker 将路径赋值给正确的字段
                if let Some(id) = self.active_picker.take() {
                    if id == 1 {
                        self.file_path1 = Some(path.clone());
                        // 自动提取元数据（在后台线程中）
                        self.extract_metadata_async(path);
                    } else if id == 2 {
    ...
    /// 在后台线程中提取媒体文件元数据
    fn extract_metadata_async(&self, path: std::path::PathBuf) {
        let sender = self.inbox.sender();
        std::thread::spawn(move || {
            let result = crate::core::utils::metadata::extract_metadata(&path);
            let _ = sender.send(UiMessages::MetadataLoaded(result));
        });
    }
